...
class TransformationService:
    ...
    async def process_transformation(user_id: int, amount: Decimal, fixing_price: Decimal, direction: str = "to_gold") -> Dict[str, Any]:
        ...
        logger.info(f"Inizio trasformazione {direction} - Utente: {user_id} - Importo: {amount} - Fixing: {fixing_price}", 
        ...
        try:
            ...
            verification_result = await TransformationService.verify_transfer(user_id, amount)
            ...
            logger.info(f"Calcolo commissioni per importo {amount}€")
            ...
            async with db.session() as session:
                async with session.begin():
                    ...
                    if direction == "to_gold":
                        user.money_account.balance -= amount
                        user.gold_account.balance += gold_amount
                    elif direction == "to_euro":
                        user.gold_account.balance -= amount
                        user.money_account.balance += gold_amount * fixing_price #Corrected calculation here.
